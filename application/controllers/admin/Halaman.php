...
class Halaman extends CIF_Controller {
    ...
    public function index($offset = 0) {
        ...
        $config['base_url'] = site_url('admin/halaman/index');
        ...
        $data['items'] = $this->db
                        ->select('halaman.*')->order_by('title')->get('halaman')->result();
    ...
    public function check() {
        ...
        if ($this->uri->segment('4')) {
            $this->db->where('halaman_id !=', $this->uri->segment('4'));
        }
        if ($this->db->where('permalink', $this->input->post('permalink'))->get('halaman')->row())
            return false;
        else
            return true;
    }
